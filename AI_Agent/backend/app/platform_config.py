...
def apply_platform_models(settings: Any, *, agent_name: str = AGENT_NAME) -> Any:
    # Docker / 本地以 AI_Agent/.env 为模型 SSOT 时，关闭平台覆盖
    if str(os.getenv("CLAWHIVE_CONFIG_SYNC") or "1").strip() == "0":
        return settings
    refresh_platform_model_cache(agent_name=agent_name)
    if not _cache.get("configured"):
        return settings
    executor = str(_cache.get("executor") or "").strip()
    if executor and hasattr(settings, "llm_model"):
        settings.llm_model = executor
    return settings
