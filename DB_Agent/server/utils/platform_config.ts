/** 从 ClawHive 拉取 DB_Agent 统一配置（能力层 + Agent 模型）。 */
...
export async function getDbAgentPlatformPayload(): Promise<PlatformConfigPayload | null> {
...
export async function getDbAgentPlatformConfig(): Promise<PlatformAgentConfigRow | null> {
  const data = await getDbAgentPlatformPayload()
  return (data?.agents || []).find((a) => String(a?.agent_name || a?.name || '') === 'DB_Agent') || null
}

const ENV_TO_RUNTIME: Record<string, string> = {
  OPENAI_ORCHESTRATION_MODEL: 'openaiOrchestrationModel',
  OPENAI_NLU_MODEL: 'openaiNluModel',
  OPENAI_AGENT_MODEL: 'openaiAgentModel',
  OPENAI_MODEL: 'openaiModel',
  EMBEDDING_MODEL: 'openaiEmbeddingModel',
}

export async function applyPlatformModelOverrides<T extends Record<string, unknown>>(config: T): Promise<T> {
  const payload = await getDbAgentPlatformPayload()
  const row = (payload?.agents || []).find((a) => String(a?.agent_name || a?.name || '') === 'DB_Agent') || null

  if (payload?.capability_configured && row?.resolved_env_models && Object.keys(row.resolved_env_models).length) {
    ...
    return { ...config, ...patch } as T
  }

  if (!row || !isPlatformModelOverrideActive(row)) return config
  ...
  return {
    ...config,
    ...(executor ? { openaiAgentModel: executor } : {}),
    ...(planner ? { openaiOrchestrationModel: planner, openaiNluModel: planner } : {}),
