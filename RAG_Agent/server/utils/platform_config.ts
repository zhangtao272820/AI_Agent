/** 从 ClawHive 拉取 RAG_Agent 统一配置（能力层 + Agent 模型）。 */
...
export async function getRagAgentPlatformConfig(): Promise<PlatformAgentConfigRow | null> {
  const data = await getPayload()
  return (data?.agents || []).find((a) => String(a?.agent_name || a?.name || '') === 'RAG_Agent') || null
}

function applyProcessEnvPatch(patch: Record<string, string>) {
...
export async function applyPlatformModelOverrides<T extends Record<string, unknown>>(config: T): Promise<T> {
  ...
  const row = (payload?.agents || []).find((a) => String(a?.agent_name || a?.name || '') === 'RAG_Agent') || null
  ...
  if (payload?.capability_configured && row?.resolved_env_models) {
    ...
    for (const [envKey, val] of Object.entries(row.resolved_env_models)) {
      if (String(val || '').trim()) patch[envKey] = String(val).trim()
    }
    if (Object.keys(patch).length) {
      applyProcessEnvPatch(patch)
  ...
  return {
    ...config,
    ...(executor ? { chatModel: executor, openaiModel: executor } : {}),
    ...(planner ? { condenseModel: planner, queryPlanModel: planner } : {}),
